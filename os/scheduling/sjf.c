...
int main() {
    ...
    for (i = 0; i < n; i++) {
        ...
        scanf("%d %d %d", &processes[i].id, &processes[i].at, &processes[i].bt);
    }
    ...
    // Apply Shortest Job First (SJF) Scheduling
    for (i = 0; i < n; i++) {
        int min = i;
        for (j = i + 1; j < n; j++) {
            if (processes[j].at <= current_time && processes[j].bt < processes[min].bt) {
                min = j;
            }
        }
        
        // Swap the process with the minimum burst time to position i
        temp = processes[i];
